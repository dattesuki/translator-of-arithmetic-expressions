...
struct Letters {
	...
	unsigned int UpperCaseDefined : 26;
	unsigned int LowerCaseDefined : 26;
	Letters() :UpperCase(0), LowerCase(0), UpperCaseDefined(0), LowerCaseDefined(0){};
};
...
Vector<Term*> String_To_Terms_with_few_expressions(std::string input_string) {
	Vector<Term*> terms;
	static Letters A;
	...
	char temp_letter;
	double temp_cin;
	static std::map<char, double> MapLetter;
	if ((A.LowerCase != A.LowerCaseDefined) || (A.UpperCase != A.UpperCaseDefined)) {
		std::cout << "\nInitialize the variables:\n";
		for (int i = 0; i < 26; ++i) {
			if (((A.UpperCase & (1 << i)) != 0) && ((A.UpperCaseDefined & (1 << i)) == 0)) {
				std::cout << char('A' + i) << " = ";
				std::cin >> temp_cin;
				MapLetter[char('A' + i)] = temp_cin;
				A.UpperCaseDefined |= (1 << i);
			}
		}
		for (int i = 0; i < 26; ++i) {
			if (((A.LowerCase & (1 << i)) != 0) && ((A.LowerCaseDefined & (1 << i)) == 0)) {
				std::cout << char('a' + i) << " = ";
				std::cin >> temp_cin;
				MapLetter[char('a' + i)] = temp_cin;
				A.LowerCaseDefined |= (1 << i);
			}
		}
	}
